...
impl<D> Default for ArrayBackedIntervalTree<D> {
    fn default() -> Self {
...
#[derive(Clone, Eq, PartialEq, Hash, Debug, Serialize, Deserialize)]
pub struct ArrayBackedIntervalTree<D> {
    entries: Vec<InternalEntry<i32, D>>,
    max_level: usize,
    indexed: bool,
}

impl<D, V> FromIterator<(V, D)> for ArrayBackedIntervalTree<D>
where
    V: Into<Interval<i32>>,
    D: Clone,
{
...
impl<D: Clone> ArrayBackedIntervalTree<D> {
    pub fn new() -> Self {
        Default::default()
    }

    pub fn insert<I: Into<Interval<i32>>>(&mut self, interval: I, data: D) {
    ...
    pub fn find<I: Into<Interval<i32>>>(&self, interval: I) -> Vec<Entry<i32, D>> {
        let mut buf = Vec::with_capacity(512);
        self.find_into(interval, &mut buf);
        buf
    }

    pub fn query<F>(&self, pos_start: i32, pos_end: i32, mut visit: F)
    where
        F: FnMut(&Interval<i32>),
    {
        self.find_visit(pos_start, pos_end, &mut visit);
    }
    ...
    ///
    /// Panics if this `IITree` instance has not been indexed yet.
    pub fn find_into<'b, 'a: 'b, I: Into<Interval<i32>>>(
        &'a self,
        interval: I,
        results: &'b mut Vec<Entry<'a, i32, D>>,
    ) {
        if !self.indexed {
    ...
    pub fn find_visit<F>(&self, pos_start: i32, pos_end: i32, visit: &mut F)
    where
        F: FnMut(&Interval<i32>),
    {
        if !self.indexed {
        ...
        // let interval = interval.into();
        let (start, end) = (pos_start, pos_end);
        let n = self.entries.len() as usize;
        let a = &self.entries;
        let mut stack = [StackCell::empty(); 64];
        // push the root; this is a top down traversal
