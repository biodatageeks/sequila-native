//! # COITrees
//! `qacoitrees` implements a fast static interval tree data structure with genomic
//! data in mind.
//!
...
use coitrees::{GenericInterval, IntWithMax, Interval, IntervalTree, SortedQuerent};
...
/// Internal interval node representation used by QABasicCOITree
#[derive(Clone)]
...
impl<'a, T, I> IntervalTree<'a> for QABasicCOITree<T, I>
where
    T: Clone + 'a,
    I: IntWithMax + 'a,
{
    ...
    type Iter = QABasicCOITreeIterator<'a, T, I>;

    fn new<'c, U, V>(intervals: U) -> QABasicCOITree<T, I>
    where
        ...
        V: GenericInterval<T> + 'c,
    {
        ...
        if nodes.len() >= (I::MAX).to_usize() {
            panic!(
                "QABasicCOITree construction failed: more intervals than index type can enumerate"
            )
        }
    ...
    fn iter(&'a self) -> QABasicCOITreeIterator<'a, T, I> {
...
impl<'a, T, I> IntoIterator for &'a QABasicCOITree<T, I>
where
    T: Clone,
    I: IntWithMax,
{
    type Item = Interval<&'a T>;
    type IntoIter = QABasicCOITreeIterator<'a, T, I>;

    fn into_iter(self) -> QABasicCOITreeIterator<'a, T, I> {
        self.iter()
    }
}

/// Iterate through nodes in a tree in sorted order by interval start position.
pub struct QABasicCOITreeIterator<'a, T, I>
where
    T: Clone,
    I: IntWithMax,
{
...
impl<'a, T, I> Iterator for QABasicCOITreeIterator<'a, T, I>
where
    T: Clone,
    I: IntWithMax,
{
...
impl<'a, T, I> ExactSizeIterator for QABasicCOITreeIterator<'a, T, I>
where
    T: Clone,
    I: IntWithMax,
{
    ...
    I: IntWithMax,
{
    ...
    type Item = IntervalNode<T, I>;
    type Iter = QABasicCOITreeIterator<'a, T, I>;
    type Tree = QABasicCOITree<T, I>;

    /// Construct a new `SortedQuerent` to perform a sequence. queries.
    fn new(tree: &'a QABasicCOITree<T, I>) -> BasicSortedQuerent<'a, T, I> {
