...
use arrow::array::{
    downcast_array, new_null_array, Array, BooleanBufferBuilder, UInt32Array, UInt32BufferBuilder,
    UInt32Builder, UInt64Array, UInt64BufferBuilder,
};
...
use datafusion::common::{plan_err, DataFusionError, JoinSide, JoinType, Result, SharedResult};
use datafusion::physical_expr::equivalence::add_offset_to_expr;
use datafusion::physical_expr::expressions::Column;
use datafusion::physical_expr::utils::{collect_columns, merge_vectors};
...
pub trait JoinHashMapType {
    ...
    fn get_matched_indices<'a>(
        ...
        for (row_idx, hash_value) in iter {
            ...
            if let Some((_, index)) = hash_map.get(*hash_value, |(hash, _)| *hash_value == *hash) {
                let mut i = *index - 1;
                loop {
                    let match_row_idx = if let Some(offset) = deleted_offset {
                        // This arguments means that we prune the next index way before here.
...
pub fn partitioned_join_output_partitioning(
    ...
    match join_type {
        ...
        JoinType::Right => adjust_right_output_partitioning(right_partitioning, left_columns_len),
...
pub fn calculate_join_output_ordering(
    ...
    let mut right_ordering = match join_type {
        // In the case below, right ordering should be offseted with the left
        // side length, since we append the right table to the left table.
        JoinType::Inner | JoinType::Left | JoinType::Right | JoinType::Full => right_ordering
            .iter()
            .map(|sort_expr| PhysicalSortExpr {
...
pub(crate) fn apply_join_filter_to_indices(
    ...
    filter: &datafusion::physical_plan::joins::utils::JoinFilter,
    build_side: JoinSide,
...
macro_rules! handle_state {
    ($match_case:expr) => {
        match $match_case {
            ...
            Ok(StatefulStreamResult::Ready(result)) => Poll::Ready(Ok(result).transpose()),
...
mod tests {
    ...
    use datafusion::common::stats::Precision::{Absent, Inexact};

    use datafusion::common::stats::Precision::{Absent, Exact, Inexact};
    use datafusion::common::{arrow_datafusion_err, arrow_err, ScalarValue};
    ...
    async fn check_error_nesting() {
        ...
        let _expected = arrow_datafusion_err!(ArrowError::CsvError("some error".to_owned()));
