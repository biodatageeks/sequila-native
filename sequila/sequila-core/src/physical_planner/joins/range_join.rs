...
mod tests {
    ...
    use super::*;
    use datafusion::{
        common::hash_utils::create_hashes, common::*, physical_expr::expressions::Column,
        physical_plan::memory::MemoryExec, physical_plan::repartition::RepartitionExec,
        physical_plan::test::build_table_i32, physical_plan::test::exec::MockExec,
    };

    use arrow::array::{ArrayRef, Date32Array, Int32Array, UInt32Builder, UInt64Builder};
    use arrow::datatypes::{DataType, Field, Schema};
    use datafusion::common::{
        assert_batches_eq, assert_batches_sorted_eq, assert_contains, exec_err, ScalarValue,
    };
    ...
    async fn partitioned_join_collect(
        ...
        let join = IntervalSearchJoinExec::try_new(
        ...
        for i in 0..partition_count {
            ...
            let more_batches = datafusion::physical_plan::common::collect(stream).await?;
            batches.extend(
                more_batches
                    .into_iter()
    ...
    async fn join_inner_one_two_parts_left(batch_size: usize) -> Result<()> {
        ...
        let left =
            Arc::new(MemoryExec::try_new(&[vec![batch1], vec![batch2]], schema, None).unwrap());
    ...
    async fn partitioned_join_right_one(batch_size: usize) -> Result<()> {
        ...
        let (columns, batches) =
            partitioned_join_collect(left, right, on, &JoinType::Right, false, task_ctx).await?;
    ...
    fn join_with_hash_collision() -> Result<()> {
        ...
        let hashes = create_hashes(&[left.columns()[0].clone()], &random_state, hashes_buff)?;
    ...
    async fn join_with_error_right() {
        ...
        for join_type in join_types {
            ...
            let result_string = datafusion::physical_plan::common::collect(stream)
                .await
                .unwrap_err()
                .to_string();
            assert!(
    ...
    async fn join_splitted_batch() {
        ...
        for join_type in join_types {
            for batch_size in (1..21).rev() {
                ...
                let join =
                    join(left.clone(), right.clone(), on.clone(), &join_type, false).unwrap();
